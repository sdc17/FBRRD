...
_u32 CacheSim::min_32(_u32 a, _u32 b){
    return a < b ? a : b;
}

_u32 CacheSim::max_32(_u32 a, _u32 b){
    return a > b ? a : b;
}

void CacheSim::set_M(int m) {
    SRRIP_M = m;
    _u32 base = pow_int(2, SRRIP_M);
    SRRIP_2_M_1 = base - 1;
    SRRIP_2_M_2 = base - 2;
}
   ...
   //如果输入配置不符合要求
    ...
    for (int i = 0; i < cache_set_size; ++i) 
        for (int j = 0; j < cache_mapping_ways; ++j) {
            ...
            caches[i * cache_mapping_ways + j].FBR = 0;
        }
...
void CacheSim::cache_hit(_u64 set_base, _u64 index, int a_swap_style) {
        ...
        switch (a_swap_style) {
                case CACHE_SWAP_LRU:
                    ...
                    break;
                case CACHE_SWAP_FBRRD:

                    // hit happend in old area then FBR += 2
                    // if(caches[set_base + index].LRU >= cache_mapping_ways - FBR_old_size){
                    //     caches[set_base + index].FBR += 2;
                    // }
                    // else if(caches[set_base + index].LRU >= FBR_new_size){
                    //     caches[set_base + index].FBR++;
                    // }

                    if(caches[set_base + index].LRU >= FBR_new_size){
                        caches[set_base + index].FBR++;
                    }
                    for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                        if ((caches[set_base + j].LRU < caches[set_base + index].LRU) && (caches[set_base + j].flag & CACHE_FLAG_VALID)) {
...
void CacheSim::cache_insert(_u64 set_base, _u64 index, int a_swap_style) {
        ...
        switch (a_swap_style) {
                ...
                case CACHE_SWAP_FBRRD:
                    ...
                    // Insert into the begin of the new area
                    // for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                    //     if ((caches[set_base + j].LRU < caches[set_base + index].LRU) && (caches[set_base + j].flag & CACHE_FLAG_VALID)) {
                    //         caches[set_base + j].LRU ++;
                    //     }
                    // }
                    // caches[set_base + index].LRU = 0;
                    // caches[set_base + index].FBR = 1;
                    // break;
                    
                    // Insert into the end of the new area
                    rand_num =  (double)rand() / RAND_MAX;
                    // insert_pos = (rand_num > EPSILON_FBR) ? (FBR_new_size - 1) : (FBR_new_size - 2);

                    // insert_pos = (rand_num > EPSILON_FBR) ? (FBR_new_size - 2) : (FBR_new_size - 3);

                    // if (rand_num > 0.75) insert_pos = FBR_new_size - 1;
                    // else if(rand_num > 0.5) insert_pos = FBR_new_size - 2;
                    // else if(rand_num > 0.25) insert_pos = FBR_new_size - 3;
                    // else insert_pos = FBR_new_size - 4;

                    insert_pos = FBR_new_size / 2;
                    for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                        if ((caches[set_base + j].LRU >= insert_pos) && (caches[set_base + j].LRU < caches[set_base + index].LRU) && (caches[set_base + j].flag & CACHE_FLAG_VALID)) {
                            caches[set_base + j].LRU ++;
                        }
                    }
                    caches[set_base + index].LRU = insert_pos;
                    caches[set_base + index].FBR = 1;
                    break;

                    // insert into the begin of the middle area
                    // for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                    //     // if ((caches[set_base + j].LRU >= cache_mapping_ways - FBR_old_size) && (caches[set_base + j].LRU < caches[set_base + index].LRU) && (caches[set_base + j].flag & CACHE_FLAG_VALID)) {
                    //         caches[set_base + j].LRU ++;
                    //     }
                    // }
                    // caches[set_base + index].LRU = cache_mapping_ways - FBR_old_size;
                    // caches[set_base + index].FBR = 1;
                    // break;
...
int CacheSim::cache_find_victim(_u64 set_base , int a_swap_style, int hit_index) {
    ...
    switch (a_swap_style) {
        ...
        case CACHE_SWAP_FBRRD:
            ...
            // normal
            // min_FBR = get_ulonglong_max();
            // for (_u64 j = 0; j < cache_mapping_ways; ++j) {
            //     if ((caches[set_base + j].LRU >= (cache_mapping_ways - FBR_old_size)) && (caches[set_base + j].FBR <= min_FBR)){
            //         min_FBR = caches[set_base + j].FBR; 
            //         free_index = j;
            //     }
            // }
            // break;

            // decrease by a specified ration
            for (_u64 j = 0; j < cache_mapping_ways; ++j) {
                if(caches[set_base + j].LRU == cache_mapping_ways - 1){
                    rand_num =  (double)rand() / RAND_MAX;
                    if((rand_num < FRB_LAST) && caches[set_base + j].FBR >=2){
                        caches[set_base + j].FBR--; 
                    }
                    continue;
                }
                else if (caches[set_base + j].LRU >= (cache_mapping_ways - FBR_old_size)){
                    rand_num =  (double)rand() / RAND_MAX;
                    if((rand_num < FBR_DECAY) && caches[set_base + j].FBR >=2){
                        caches[set_base + j].FBR--; 
                    }
                }
            }
            ...
            // decrease FBR every time;
            // max_LRU = 0;
            // while(free_index < 0){
            //     for (_u64 j = 0; j < cache_mapping_ways; ++j) {
            //         if ((caches[set_base + j].LRU >= (cache_mapping_ways - FBR_old_size)) && (caches[set_base + j].FBR == 1) && (caches[set_base + j].LRU >= max_LRU)){
            //             free_index = j;
            //             max_LRU = caches[set_base + j].LRU;
            //             // break;
            //         }
            ...
            //     if(free_index < 0){
            //         for (_u64 j = 0; j < cache_mapping_ways; ++j) {
            //             if (caches[set_base + j].LRU >= (cache_mapping_ways - FBR_old_size)){
            //                 caches[set_base + j].FBR--;
            //             }
            //         }
            //     }
...
void CacheSim::load_trace(const char *filename) {
    ...
    while (fgets(buf, sizeof(buf), fin)) {
        ...
        if (mid == 0x7a || mid == 0x7c) {
            cache_gpu++;
            cache_machine = CACHE_GPU;
        } else if (mid == 0x78 || mid == 0x79) {
            cache_cpu++;
            cache_machine = CACHE_CPU;
        } else {
            printf("%x", mid);
    ...
    printf("Cache from gpu/cpu rate: %f%%/%f%%\n", 100.0 * cache_gpu / (cache_gpu + cache_cpu), 100.0 * cache_cpu / (cache_gpu + cache_cpu));
    fclose(fin);
}
